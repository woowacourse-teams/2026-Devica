<!-- Devica 대표 이미지가 준비되면 아래에 이미지 URL을 추가하세요. -->
<p align="center">
  <img src="docs/images/logo.png" alt="Devica - W.R.B" width="100%">
</p>

# Devica

> 내 사용 목적에 맞는 전자기기, 더 쉽게 결정하세요

Devica는 전자기기를 구매할 때 필요한 정보가 여러 곳에 흩어져 있어 결정을 어려워하는 사용자를 위한 서비스입니다.

사용 목적에 맞는 리뷰, 리뷰 작성자의 업무 맥락과 신뢰를 판단할 정보, 전문가 유튜브 콘텐츠의 핵심 사양 정보를 한곳에서 제공합니다.

흩어진 스펙과 후기를 반복해서 찾아보는 대신, 내게 필요한 근거를 바탕으로 적합한 전자기기를 더 쉽게 결정해보세요.

**사용 목적에 맞는 정보와 신뢰할 수 있는 리뷰를 연결하는 전자기기 구매 결정 서비스**, Devica가 여러분의 구매 결정을 도와드립니다.


<br>

## 서비스 소개

### 필요한 전자기기부터 탐색해보세요 💻

<!-- <p align="center"><img src="여기에_카테고리_탐색_스크린샷_URL" width="25%"></p> -->

***필요한 기기부터, 간편하게 탐색!***

노트북, 키보드 등 필요한 전자기기 카테고리를 선택하고,
제품을 고르기 위한 기준과 리뷰를 한곳에서 확인할 수 있어요.

<br>

### 나의 직군과 사용 목적에 맞춰보세요 🎯

<!-- <p align="center"><img src="여기에_직군_선택_스크린샷_URL" width="25%"></p> -->

***내게 필요한 기준부터, 나의 업무 맥락에 맞게!***

프론트엔드, 백엔드, 디자인 등 자신의 직군과 사용 목적을 선택하면,
나에게 더 적합한 구매 기준과 정보를 중심으로 탐색할 수 있어요.

<br>

### 같은 일을 하는 사람의 리뷰를 확인해보세요 📝

<!-- <p align="center"><img src="여기에_직군별_리뷰_스크린샷_URL" width="25%"></p> -->

***나와 비슷한 사람의 실제 사용 경험!***

같은 직군에서 제품을 사용한 사람의 업무 환경과 장단점을 확인하고,
단편적인 후기만으로 알기 어려운 실질적인 선택 기준을 찾아보세요.

<br>

### 전문가의 핵심 정보를 빠르게 확인해보세요 ⏱️

<!-- <p align="center"><img src="여기에_전문가_타임라인_스크린샷_URL" width="25%"></p> -->

***긴 리뷰에서 필요한 내용만, 빠르게!***

전문가 리뷰의 핵심 내용을 요약해 확인하고,
필요한 구간은 타임라인을 통해 원본 콘텐츠에서 바로 살펴볼 수 있어요.

<br>

## 멤버 소개

| 와이제리(최용준) | 라텔(김규빈) | 밤밤(홍우현) |
| :---: | :---: | :---: |
| Backend | Backend | Backend |
| <a href="https://github.com/yj9107v"><img src="https://avatars.githubusercontent.com/yj9107v" width="150" alt="와이제리 GitHub 프로필"/></a> | <a href="https://github.com/bin1225"><img src="https://avatars.githubusercontent.com/bin1225" width="150" alt="라텔 GitHub 프로필"/></a> | <a href="https://github.com/Woohyeon-Hong"><img src="https://avatars.githubusercontent.com/Woohyeon-Hong" width="150" alt="밤밤 GitHub 프로필"/></a> |

<br>

## 기술 스택

### Frontend

프론트엔드 기술 스택은 확정 후 추가할 예정입니다.

<br>

### Backend

![Java](https://img.shields.io/badge/Java-21-007396?style=for-the-badge&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-4.1.0-6DB33F?style=for-the-badge&logo=springboot&logoColor=white)
![Spring Data JPA](https://img.shields.io/badge/Spring_Data_JPA-4.1.0-59666C?style=for-the-badge)
![Gradle](https://img.shields.io/badge/Gradle-9.5.1-02303A?style=for-the-badge&logo=gradle&logoColor=white)

### Database

![MySQL](https://img.shields.io/badge/MySQL-8.4.x_LTS-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![MySQL Connector/J](https://img.shields.io/badge/MySQL_Connector%2FJ-9.7.0-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

### Test

![JUnit](https://img.shields.io/badge/JUnit-6.0.3-25A162?style=for-the-badge&logo=junit5&logoColor=white)
![AssertJ](https://img.shields.io/badge/AssertJ-3.27.2-9F2B68?style=for-the-badge)

### Infrastructure

![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Docker Compose](https://img.shields.io/badge/Docker_Compose-V2-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![JAR](https://img.shields.io/badge/Packaging-JAR-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)

<br>

## Document

Devica 팀에 대한 자세한 정보는 아래 링크에 정리되어 있습니다.

🤜 [Devica Wiki](https://github.com/woowacourse-teams/2026-Devica/wiki)

📁 [프로젝트 문서](docs/README.md) — 개발 환경 세팅, 팀 컨벤션, 유스케이스, 기술 선택 근거(ADR)
